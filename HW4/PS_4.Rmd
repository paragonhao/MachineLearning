...
totalsample <- factor_return[1:25,]
fitWholeSample <- glmnet(cov(totalsample),colMeans(totalsample), family = "gaussian", alpha = 0.5, lambda = lambda_min)
...
Now we could work on expanding window for prediction, 

```{r}
outsample <- factor_return[26,]
MVE_ret_1yr <- sum(b * outsample)
